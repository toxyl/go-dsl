	...
	t.Run("Tokenizer", func(t *testing.T) {
		...
		tests := []TestCase{
			...
			c("negative numbers 2", `test-function-1(-1.5 -2.5 "hello \" mean\"world!\"")`, false,
				tkn(`test-function-1(`, dsl.tokens.callStart), tkn(`-1.5`, dsl.tokens.float), tkn(`-2.5`, dsl.tokens.float), tkn(`hello " mean"world!"`, dsl.tokens.str), tkn(`)`, dsl.tokens.callEnd),
			),
			c("negative numbers 3", `test-function-1(-1.5 -2 "hello \" mean\"world!\"")`, false,
				tkn(`test-function-1(`, dsl.tokens.callStart), tkn(`-1.5`, dsl.tokens.float), tkn(`-2`, dsl.tokens.integer), tkn(`hello " mean"world!"`, dsl.tokens.str), tkn(`)`, dsl.tokens.callEnd),
			),
